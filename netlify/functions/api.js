...
export async function handler(event) {
  ...
  // 🏠 Trang chủ giới thiệu toàn bộ API
  if (duongdan === "" || duongdan === "home") return hienThiHome();
  ...
  return {
    statusCode: 404,
    body: JSON.stringify({
      loi: "Không tìm thấy API. Dùng /home, /bao hoặc /kiemtra",
    }),
  };
}

// =================================================
// 🏠 TRANG HOME - DANH SÁCH CHỨC NĂNG
// =================================================
async function hienThiHome() {
  const chucNang = {
    "/home": "Hiển thị toàn bộ chức năng của API",
    "/bao?nguon=vnexpress": "Lấy tin tức mới nhất từ VNExpress (hoặc ?nguon=dantri, 24h)",
    "/kiemtra?url=https://example.com":
      "Kiểm tra bảo mật website (chedo=nhanh hoặc chamsoc)",
  };
      ...
      body: JSON.stringify(
        {
          ...
          duLieu: baiBao.length
            ? baiBao
            : "Không lấy được dữ liệu (có thể bị chặn Cloudflare)",
        },
        null,
        2
      ),
...
async function quetTrangWeb(event) {
  const q = event.queryStringParameters || {};
  const rawUrl = q.url;
  const cheDo = (q.chedo || "nhanh").toLowerCase();
  ...
  try {
    const t0 = Date.now();
    const phanHoi = await axios.get(mucTieu.href, {
      timeout: 15000,
      validateStatus: null,
      headers: { "User-Agent": "Mozilla/5.0 (Netlify-Scan)" },
    });
    ketQua.tomtat = { trangThai: phanHoi.status, phanHoiMs: Date.now() - t0 };
    ...
    return { statusCode: 500, body: JSON.stringify({ loi: e.message }) };
  }
  ...
  return new Promise((resolve, reject) => {
    const socket = tls.connect(443, host, { servername: host, timeout: 8000 }, () => {
      ...
      if (!cert || Object.keys(cert).length === 0)
        return reject(new Error("Không lấy được chứng chỉ SSL"));
      resolve(cert);
    });
